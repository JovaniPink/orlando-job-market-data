...
def get_jobs_summary(site, summary_list):
    scraped_summary = summary_list
    summary_spans = site.find_all("span", attrs={"class": "summary"})
    for summary in summary_spans:
        scraped_summary.append(summary.text.strip())
...
def set_driver():
    with open("config/driver_window.json") as window:
        minimized = json.load(window).get("minimized")
...
def get_job_description(url, descriptions_list):
    ...
    def click_on_job_and_add_description(job_card):
        ...
        job_card.click()
        wait.until(EC.presence_of_element_located((By.ID, "vjs-container-iframe")))
        ...
        scraped_descriptions.append(driver.find_element_by_id("jobDescriptionText"))
